...
const AdminDashboard = () => {
    ...
    const statCards = [
        ...
        { label: 'Sellers', value: totalSeller, icon: <FaUsers />, bg: 'bg-emerald-50', iconBg: 'bg-emerald-600' },
        { label: 'Orders', value: totalOrder, icon: <FaCartShopping />, bg: 'bg-amber-50', iconBg: 'bg-amber-500' },
    ]
        ...
        <div className='w-full lg:w-5/12 lg:pl-3 mt-6 lg:mt-0'>
            <div className='w-full bg-white p-5 rounded-xl border border-slate-100 shadow-sm h-full'>
                <div className='flex justify-between items-center'>
                    <h2 className='font-bold text-base text-slate-700 pb-3'>Recent Seller Message</h2>
        ...
        <div className='flex flex-col gap-2 pt-4'>
            <ol className='relative border-l border-slate-200 ml-4'>

               {
                ...
                {
                    m.senderId === userInfo._id ? <img className='w-full rounded-full h-full' src={userInfo.image} alt="" /> : <img className='w-full rounded-full h-full' src={seller} alt="" />
                }
                </div>
        ...
        <tbody>
            {
                ...
                <td scope='row' className='py-3 px-4 font-medium whitespace-nowrap'>
                    <Link className='text-[#2563EB] hover:underline' to={`/admin/dashboard/order/details/${d._id}`}>View</Link> </td>
            </tr> )
            }
