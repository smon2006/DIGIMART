...
class cardController{
    ...
    get_card_products = async(req, res) => {
      ...
      const validCardProducts = card_products.filter(p => p.products.length > 0)

      let buy_product_item = 0
      let calculatePrice = 0;
      let card_product_count = 0;
      const outOfStockProduct = validCardProducts.filter(p => p.products[0].stock < p.quantity)
      for (let i = 0; i < outOfStockProduct.length; i++) {
         card_product_count = card_product_count + outOfStockProduct[i].quantity        
      }
      const stockProduct = validCardProducts.filter(p => p.products[0].stock >= p.quantity)
