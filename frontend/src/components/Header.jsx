...
const Header = () => {
    ...
    return (
        ...
        <div className='w-white'>
         <div className='w-[85%] lg:w-[90%] mx-auto'>
            <div className='h-[86px] md-lg:h-[100px] flex justify-between items-center flex-wrap relative'>
                ...
                <div title="Menu" className='justify-center items-center w-[36px] h-[36px] bg-white text-slate-600 border border-slate-300 rounded-lg cursor-pointer lg:hidden md-lg:flex xl:hidden hidden hover:border-[#0891B2] hover:text-[#0891B2] transition-colors' onClick={() => setShowShidebar(false)}>
                    <span> <FaList/> </span>
                </div>
                </div> 
                </div> 
            ...
            <div className='md:lg:w-full w-9/12'>
                <div className='flex justify-end md-lg:justify-center items-center flex-wrap pl-8'>
                    <ul className='flex justify-center items-start gap-8 text-[13px] font-bold uppercase tracking-wide md-lg:hidden absolute left-1/2 -translate-x-1/2'>
                        {
                            navLinks.map((n, i) => (
                                <li key={i}>
                                    <Link to={n.to} className={`relative py-2 block transition-colors after:absolute after:left-0 after:-bottom-[2px] after:h-[2px] after:bg-[#0891B2] after:transition-all ${pathname === n.match ? 'text-[#0891B2] after:w-full' : 'text-slate-100 hover:text-[#0891B2] after:w-0 hover:after:w-full'}`}>{n.label}</Link>
                                </li>
                            ))
                        }
                    </ul>
  ...
  <div title="Wishlist" onClick={() => navigate(userInfo ? '/dashboard/my-wishlist' : '/login') } className='relative flex justify-center items-center cursor-pointer w-[40px] h-[40px] rounded-full bg-slate-100 hover:bg-[#0891B2] group transition-colors duration-200'>
...

            {
                ...
                {wishlist_count}
                        ...
                        <div title="Cart" onClick={redirect_card_page} className='relative flex justify-center items-center cursor-pointer w-[40px] h-[40px] rounded-full bg-slate-100 hover:bg-[#0891B2] group transition-colors duration-200'>
    ...
    <ul className='flex flex-col justify-start items-start text-sm font-bold uppercase gap-1 w-full'>
                        {
                            navLinks.map((n, i) => (
                                <li key={i} className='w-full border-b border-slate-100 last:border-none'>
                                    <Link to={n.to} className={`py-3 block ${pathname === n.match ?  'text-[#0891B2]' : 'text-slate-600' } `}>{n.label}</Link>
                                </li>
                            ))
                        }
                    </ul>
    <div className='flex justify-start items-center gap-4 text-slate-500'>
                    <a href="https://www.linkedin.com/in/shrestha-mondal-cse/" target="_blank" rel="noopener noreferrer" title="LinkedIn" className='hover:text-[#0891B2] transition-colors'><FaLinkedin /></a>
                    <a href="https://github.com/smon2006/DIGIMART" target="_blank" rel="noopener noreferrer" title="GitHub" className='hover:text-[#0891B2] transition-colors'><FaGithub /> </a> 
        </div>

        <div className='w-full flex justify-end md-lg:justify-start gap-3 items-center'>
        <div title="Phone" className='w-[48px] h-[48px] rounded-full flex bg-slate-100 text-[#0891B2] justify-center items-center '>
    ...
    <div className='w-[85%] lg:w-[90%] mx-auto'>
        <div className='flex w-full flex-wrap md-lg:gap-8 pb-4'>
            <div className='w-3/12 md-lg:w-full'>
                <div className='bg-white relative rounded-t-lg'>
                   <div title="Browse categories" onClick={() => setCategoryShow(!categoryShow) } className='h-[48px] rounded-t-lg bg-[#0891B2] hover:bg-[#0E7490] transition-colors text-white flex justify-center md-lg:justify-between md-lg:px-6 items-center gap-3 font-semibold text-sm cursor-pointer'>
                    ...
                    categorys.map((c,i) => {
                        return (
                         <li key={i} className='flex justify-start items-center gap-3 px-[20px] py-[8px] hover:bg-slate-50 transition-colors'>
                            ...
                            <Link to={`/products?category=${c.name}`} className='text-sm block hover:text-[#0891B2] transition-colors'>{c.name}</Link>
                         </li>
                        )
                    })
                }
        ...
        <div className='w-9/12 pl-8 md-lg:pl-0 md-lg:w-full'>
            <div className='flex flex-wrap w-full items-center md-lg:gap-6'>
                <div className='w-full'>
                    <div className='flex bg-white border border-slate-300 focus-within:border-[#0891B2] focus-within:ring-2 focus-within:ring-[#0891B2]/20 rounded-full h-[48px] items-center relative gap-6 transition-all overflow-hidden'>
                        <input className='w-full relative bg-transparent text-slate-600 text-sm outline-0 pl-5 pr-3 h-full placeholder:text-slate-400' onChange={(e)=> setSearchValue(e.target.value)} type="text" name='' id='' placeholder='What do you need' />
                        <button title="Search" onClick={search} className='bg-[#0891B2] hover:bg-[#0E7490] right-0 absolute px-7 h-full font-semibold text-sm uppercase text-white flex items-center gap-2 transition-colors rounded-r-full'>
                            <FaSearch size={13} />
